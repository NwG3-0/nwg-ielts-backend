...
import { LikePostModel } from '../models/LikePost'
...
export const getLikePost = async (req, res, _next) => {
  try {
    const { newsId } = req.query
    ...
    const totalRecords = await LikePostModel.countDocuments({
      News: newsId,
    })
...
export const addViewPost = async (req, res, next) => {
  try {
    const { newsId, userId } = req.body
    ...
    const response = await LikePostModel.create({
      News: newsId,
      User: userId,
      CreatedAt: currentTimestamp,
      UpdatedAt: currentTimestamp,
...
export const checkLikePost = async (req, res, _next) => {
  try {
    const { newsId, userId } = req.query
    ...
    const response = await LikePostModel.find({
      User: userId,
      News: newsId,
    })
...
export const unLikePost = async (req, res, _next) => {
  try {
    const { newsId, usersId } = req.body
    ...
    const response = await LikePostModel.findOneAndDelete({ News: newsId, User: usersId }).catch(() => {
      res.status(StatusCodes.BAD_REQUEST).json({ success: false, data: null, message: 'You delete the post fail' })
    })
